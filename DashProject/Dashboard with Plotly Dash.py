...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider',
                                                min=0, max=10000, 
                                                step=500,
                                                marks={0:'0',2500:'2.500',
                                                5000:'5.000',7500:'7.500',
                                                10000:'10.000'},                                           
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        data = filtered_df.groupby(by='Launch Site').sum().reset_index()
        fig = px.pie(data, values='class', names='Launch Site', 
        title='Total Success Launches By Site')
        return fig
    else:
        data = filtered_df.loc[filtered_df['Launch Site'] == entered_site]
        data = data.groupby(by='class').count().reset_index()
        fig = px.pie(data, values='Launch Site', names='class',
...
def get_scatter_chart(entered_site,payload_interval):
    ...
    min_filter = payload_interval[0]
    max_filter = payload_interval[1]
    data = filtered_df[filtered_df['Payload Mass (kg)'].between(min_filter, max_filter)]
    if entered_site == 'ALL':
        ...
        data = data.loc[filtered_df['Launch Site'] == entered_site]
        fig = px.scatter(data, x='Payload Mass (kg)', y='class',
        color='Booster Version Category',
